...
class MartingaleStrategyAdvance(bt.Strategy):
    params = (
        ('start_position_size', 10.0),
        ('take_profit_percent', 15.0),
        ('martingale_percent', 3.0),
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if price_percent > (self.params.take_profit_percent * self.params.takeprofit_mult * self.count_martingale):
                self.close()
                self.first_buy_price = None
                self.last_buy_price = None
                self.count_martingale = 0
            ...
            next_buy_price = self.last_buy_price * (1 - (self.params.martingale_percent * math.pow(self.params.correction_buy_mult, self.count_martingale)/100))
            if self.data.close[0] < next_buy_price and self.data.close[0] < self.last_buy_price * (1 - self.params.martingale_percent / 100):
                unit_size_martingale = unit_size * math.pow(self.params.martingale_mult, self.count_martingale)
                self.buy(size=unit_size_martingale)
                self.last_buy_price = self.data.close[0]
...
def backtest_coin_adv(df,data_signals):
    cerebro = bt.Cerebro()
    cerebro.addstrategy(
        MartingaleStrategyAdvance,
        ...
        correction_buy_mult=data_signals["correction_buy_mult"],
        takeprofit_mult=data_signals["takeprofit_mult"]
    )
...
def backtest(df,data_signal,indicator):
    if indicator=='martingale':
        ...
        res=backtest_coin_adv(df,data_signal)
    return res